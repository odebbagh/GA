...
// Purpose: Open the Management Review attachment (OPEN URL) — resolve blob via UUID_sfwDocument / legacy document.blob, write temp file with a safe leaf name.
// Parameters: $review — cs.ManagementReviewEntity. Returns: Boolean — True after OPEN URL.
// created by 4D/PS [2026-may-08]

#DECLARE($review : cs:C1710.ManagementReviewEntity)->$ok : Boolean
...
$ok:=False:C215
If ($review=Null:C1517) || ($review.document=Null:C1517)
	cs:C1710.sfw_dialog.me.alert("No attachment is available for this management review.")
	return 
End if 

$leafSrc:=""
$uuidDoc:=String:C10($review.document.UUID_sfwDocument)
...
If (($blob=Null:C1517) || (BLOB size:C605($blob)=0)) && (OB Is defined:C1231($review.document; "blob"))
	$blob:=$review.document.blob
	If ($ext="")
		$ext:=Lowercase:C14(String:C10($review.document.extension))
	End if 
	If ($leafSrc="")
		$leafSrc:=String:C10($review.document.sourcePath)
	End if 
End if 
...
End if 
If ($baseLeaf="")
	$baseLeaf:="ManagementReview_"+String:C10($review.managementReviewNumber)
End if 
