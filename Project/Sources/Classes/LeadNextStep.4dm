...
local Function entryDefinition()->$entry : cs:C1710.sfw_definitionEntry
	...
	$entry:=cs:C1710.sfw_definitionEntry.new("leadNextStep"; "administration"; "Lead next steps")
	...
	$entry.setPanel("panel_leadNextStep")
	
	$entry.setLBItemsColumn("colorPicto"; ""; "width:20"; "type:picture")
	$entry.setLBItemsColumn("nextStepID"; "ID"; "width:30")
	$entry.setLBItemsColumn("code"; "Code"; "width:80")
	$entry.setLBItemsColumn("name"; "Name"; "width:200")
	
	$entry.setLBItemsOrderBy("nextStepID")
	$entry.setLBItemsOrderBy("code")
	
	$entry.setLBItemsCounter("###0###0##0^1;;"; "unit1:step"; "unitN:steps")
	
	$entry.setValidationRule("nextStepID"; "entryField_stageID"; "mandatory"; "trimSpace"; "capitalize")
...
local Function cacheLoad()
	...
	If (Storage:C1525.cache=Null:C1517)
		...
		End use 
	End if 
	If (Storage:C1525.cache.leadNextStep=Null:C1517)
		$leadNextStepColl:=This:C1470._loadAsCollection()
		Use (Storage:C1525.cache)
			Storage:C1525.cache.leadNextStep:=$leadNextStepColl.copy(ck shared:K85:29; Storage:C1525.cache)
		End use 
	End if 
...
local Function cacheGet($uuid : Text)->$leadStage : Object
	...
	Else 
		...
		End if 
	End if 
	$indices:=Storage:C1525.cache.leadNextStep.indices("UUID = :1"; $uuid)
	If ($indices.length>0)
		$leadStage:=Storage:C1525.cache.leadNextStep[$indices[0]]
	Else 
...
Function trigger()
	...
	Else 
		EXECUTE ON CLIENT:C651("@"; "sfw_cacheManager"; "clear"; "LeadNextStep")
	End if 
	
Function _loadAsCollection()->$projectStatusColl : Collection
	var $file : 4D:C1709.File
	var $img : Picture
	$projectStatusColl:=This:C1470.all().toCollection("UUID, nextStepID, code, name, color").orderBy("nextStepID")